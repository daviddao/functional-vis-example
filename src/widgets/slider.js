...
function BioJsSlider(opts) {
	...
	this.text.onValue(function (text) {
		label.text(text);
	});

	/* keep the value label up to date with the model */
	this.value.onValue(function (v) {
		value.text(v);
	});

	/* keep the value consistent with the min, max and step */
	this.value.plug(Kefir.combine([this.value, this.min]).filter(function (A) {
		return A[0] < A[1];
	}).map(function (A) {
		return A[1];
	}));
	this.value.plug(Kefir.combine([this.value, this.max]).filter(function (A) {
		return A[0] > A[1];
	}).map(function (A) {
		return A[1];
	}));
	...
	["min", "max", "step", "value"].forEach(function (prop) {
		_this[prop].onValue(function (v) {
			slider.prop(prop, v);
		});
	});
	...
	slider.on("change input", function () {
		_this.value.emit(parseInt(slider.val(), 10));
	});
}